...
|    8 | 备份/恢复/切换     | 进行中 | 本阶段提交，主题 `feat: add verified backup operations`  |
...
与 `docs/reports/**`；修改 app/startup data lock、洞察查询、workspace scripts、Data page、E2E 和基线文档；
删除文件：无。
...
| `npm run test:coverage` | 通过 | 51 files、297 tests（Server 202、Web 38、Shared 57），0 failed、0 skipped                         |
| 全局覆盖率              | 通过 | Server 96.94/87.64/97.31；Web 96.29/89.98/88.83；Shared 100/98.52/100（lines/branches/functions） |
...
| `npm run check:all`     | 通过 | format、lint、typecheck、297 tests、build、10 Chromium E2E 全部通过                               |
| 性能审计                | 通过 | fixture build 约 0.27s；7 查询约 0.2–124ms；查询计划均使用索引或至多 1 行 CTE                     |
...
- 性能数据来自本机合成 fixture；真实长文本、更多分P或较慢磁盘仍需 3–7 天窗口观察。
...
- 阶段 8 工程实现和可自动执行的验收已完成：备份/恢复、安全矩阵、真实性能 fixture、查询计划、
  qoder 脱敏导入、跨进程恢复、文档、干净安装、覆盖率和 `check:all` 均通过。
- 阶段 8 尚未完成：真实 Personal 导出、3–7 天并行使用、用户核心工作流确认和旧项目 30 天保留期
  不能由自动测试代替。上述条件全部满足前，按执行规范不得宣布 vNext 完成。
