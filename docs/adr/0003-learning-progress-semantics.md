...
- Status: Accepted（回顾页时长口径部分已被 ADR 0006 取代）
...
> 注：本节中「回顾区间统计只汇总 `learning_watch_daily`」的口径已被 ADR 0006 取代
> （回顾页观看进度改为按系列实时汇总）；`learning_watch_daily` 仅作为每日实际观看的
> 历史记录保留。时长封顶估算规则本身仍有效。
