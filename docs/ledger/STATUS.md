...
- 阶段：8（完成）；此后为常规迭代，当前版本 **v1.10.0**（2026-09-18 已发布：
  <https://github.com/quexing65/workbench/releases/tag/v1.10.0>）。
...
- v1.7.0 为设置页、暗色主题、全局快捷键、逾期批量操作、toast 反馈、总览近 7 天趋势、
  任务日期快捷切换、学习页筛选排序与 mm:ss 进度录入的功能版，同样按「桌面应用发布」
  流程完成：main CI 全绿（run `34520481694`）→ tag `v1.7.0` 打在 `91bb6db` → Release
  流水线 `34521023941` 构建 NSIS/便携版并生成草稿 → 草稿补写变更摘要与校验和表后正式
  发布 → 已在 `docs/operations/RELEASES.md` 登记校验和。
- v1.8.0 为学习资源自定义标题（schema v6）、学习页布局瘦身与手动进度录入退役的版本，
  同样按「桌面应用发布」流程完成：main CI 全绿（run `34776441152`）→ tag `v1.8.0` 打在
  `54a7f6c` → Release 流水线 `34776727035` 构建 NSIS/便携版并生成草稿 → 草稿补写变更
  摘要与校验和表后正式发布 → 已在 `docs/operations/RELEASES.md` 登记校验和。
- v1.8.1 为应用图标重设计的补丁版，同样按「桌面应用发布」流程完成：main CI 全绿
  （run `34802524579`）→ tag `v1.8.1` 打在 `15d12a8` → Release 流水线 `34802841002`
  构建 NSIS/便携版并生成草稿 → 解包核对 CI 产物内嵌图标确为新样式 → 草稿补写变更
  摘要与校验和表后正式发布 → 已在 `docs/operations/RELEASES.md` 登记校验和。
- v1.8.2 为桌面退出端口占用等五处缺陷修复的补丁版（一次全仓 bug 审计的产出），同样
  按「桌面应用发布」流程完成：main CI 全绿（run `34810839892`）→ tag `v1.8.2` 打在
